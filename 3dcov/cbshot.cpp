...
void cbshot::calculate_cov(pcl::PointCloud<pcl::PointXYZ>& cloud_in, pcl::PointCloud<pcl::PointXYZ>& cloud_out, Eigen::Matrix4f & iG_transf,Eigen::Matrix4f & f_transform,Eigen::MatrixXd & iG_cov)
{
  
     Eigen::MatrixXd ICP_COV(6,6);
     ICP_COV = Eigen::MatrixXd::Zero(6,6); 
     f_transform = iG_transf;

     calculate_ICP_COV(cloud_in, cloud_out, iG_transf, ICP_COV); 
     iG_cov = ICP_COV;
}



void cbshot::calculate_ransac(pcl::PointCloud<pcl::PointXYZ>& cloud_in, pcl::PointCloud<pcl::PointXYZ>& cloud_out, Eigen::Matrix4f & iG_transf)
{
    ...
    if (file.is_open())
	{
		file << "RANSAC based Transformation Matrix : \n" << ransac_mat << '\n';
	}
    iG_transf = ransac_mat;
